...
const stripePromise = loadStripe("pk_test_51MKxDESEKxefYE6MZCHxEw4cFKiiLn2mV3Ek4Nx1UfcuNfE1Z6jgQrZrKpqTLju3n5SBjYJcwt1Jkw1bEoPXWRHB00XZ7D2f2F")
                        ...
                        if (result.status === 1) {
                            await api.initiate_transaction(cookies.get('jwt_token'), result.data.order_id, "Razorpay")
                                ...
                                .then(res => {
                                    ...
                                    if (res.status === 1) {
                                        setloadingPlaceOrder(false)
                                        handleRozarpayPayment(result.data.order_id, res.data.transaction_id, cart.checkout.total_amount, user.user.name, user.user.email, user.user.mobile, setting.setting.app_name);
                                    }
                                    else {
                ...
                await api.placeOrder(cookies.get('jwt_token'), cart.checkout.product_variant_id, cart.checkout.quantity, cart.checkout.sub_total, cart.checkout.delivery_charge.total_delivery_charge, cart.checkout.total_amount, paymentMethod, selectedAddress.id, delivery_time)
                    ...
                    .then(result => {
                        ...
                        if (result.status === 1) {
                            ...
                            handlePayStackPayment(user.user.email, cart.checkout.total_amount, setting.payment_setting.paystack_currency_code, setting.setting.support_email)
                                                                ...
                                                                : <>
                                                                    <div className='button-container'>
                                                                        {paymentMethod === "Stripe"
                                                                            ? <motion.button whileTap={{ scale: 0.8 }} type='button' className='checkout' onClick={(e) => { e.preventDefault(); HandlePlaceOrder() }} data-bs-toggle="modal" data-bs-target="#stripeModal">place order</motion.button>
                ...
                <div className="modal-dialog modal-dialog-centered modal-lg">
                    <div className="modal-content" style={{ minWidth: "100%" }}>
                        {stripeOrderId === null || stripeClientSecret === null || stripeTransactionId === null
                            ...
                            :
                            <Elements stripe={stripePromise} orderID={stripeOrderId} client_secret={stripeClientSecret} transaction_id={stripeTransactionId} amount={cart.checkout.total_amount}>
                                <InjectCheckout orderID={stripeOrderId} client_secret={stripeClientSecret} transaction_id={stripeTransactionId} amount={cart.checkout.total_amount} />
                            </Elements>
                        }
